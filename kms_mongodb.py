...
def delete_document():
    user_id = user_id_entry.get()
    result = collection.delete_many({'user.user_id': user_id})
...
def get_user_activities():
    user_id = user_id_entry.get()
    activities = list(collection.find({'user.user_id': user_id}))
    ...
    for activity in activities:
        ...
        activities_listbox.insert(tk.END, f"تاریخ: {publish_date} | توضیحات: {description}")
